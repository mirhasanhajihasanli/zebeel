...
def read_gps(gps_serial):
    ...
    if "$GPGGA" in line:
        ...
        try:
            ...
            return int(latitude), int(longitude)  # Integer olarak döndür
            lastlat = latitude
            lastlon = longitude
        except (ValueError, IndexError):
            pass
    return lastlat, lastlon  # Hata durumunda sıfır değerleri döndür
...
def main():
    ...
    try:
        while True:
            ...
            # Mesaj oluştur
            message = f"<START>D:{distance}cm,L:{latitude},Ln:{longitude}<END>"
